...
class PostVoteMapper extends Mapper{
	...
	public function savePostVote($postId , $vote , $uid ){
	...
	public function getCastedVotes($postId){
		
		$userVotesArr = $voteVolumArr = array();
		
		$rS = $this->query("SELECT vote FROM ".$this->getTable()." WHERE post_id='$postId' ");
	...
	public function getMeterPersistant($postId){
		
		$meterIcon = '';
		$castedVotesArr = $this->getCastedVotes($postId);
	...
	public function isUserVoted($uid , $postId){
		$rS = $this->query("SELECT id FROM ".$this->getTable()." WHERE uid='$uid' AND post_id='$postId'");
