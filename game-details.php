...
<?php
	...
	$gameId      = $_GET['id'];
	$gameInfoArr = $mapObj->fetchById($gameId);
	
	$gameIcon = ArrayUtil::value('icon_image',$gameInfoArr);
	if($gameIcon != ''){
		$gameIcon = $mapObj->getUploadLoc().$gameIcon;
	}else{
		
	}
	
	$gamMap  = new GameCommentsMapper();
	$totalComments = $gamMap->countByGame($gameId);
?>
...
<script type="text/javascript">
	$(document).ready(function(){
		loadAllComments( 'game' , '<?=$gameId?>' ,'DESC');		
	});
</script>
<style type="text/css">
...
<body>
    ...
    <div class="contentWrapper">
       ...
       <h1 class="content-heading">Games</h1>
       <div class="left-content-container">
       		
			<?=$mapObj->drawGameDetails($gameId, $boot)?>
			<br />
            <?=Block::drawCommentsFrm('game','gid',$gameId , $boot->isLogined());?>
                        
            <br /><br /><br /><br />
            
            <div class="comments-container">
            	<div class="comments-total" id="post-detail-no-comments">
                	<?=$mapObj->getTotalGameDetailComments($gameId);?>
                </div>
       ...
       <div class="right-content-container">
			...
			<?=Block::drawFollowerUser( $boot, $gameId  , 'game')?>
